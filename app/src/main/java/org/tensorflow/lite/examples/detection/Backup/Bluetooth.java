...
//import org.tensorflow.lite.examples.detection.Backup.work_information;
...
//
//        wi.set_work(1,0,"", "","2관의 시작 지점입니다.");
//        wi.set_work(1,1,"쇼룸 1", "김지은","모델하우스처럼 잘 꾸며진 침실입니다. 최신 유행을 따르면서도 무난한 스타일을 꾸며진 이 공간을 자세히 살펴보면 침구나 카펫, 가구에 이케아 카탈로그에서 수집한 실내 사진이 꼴라주되어 있음을 알 수 있습니다. 공간 너머엔 작가가 살고 있는 아파트 단지가 보입니다. 작가는 밀집된 대단지 아파트의 모습에서 바코드를 연상합니다. 언제나 시세를 확인할 수 있는 아파트에서의 삶에 대해 사적인 공간이 아닌, ‘가판대에 나와 있는 것 같다’라고 비유합니다. ");
//        wi.set_work(1,2,"아파트 컬러 트렌드 VS 르 꼬르뷔지에", "김지은","아파트 컬러에도 트렌드가 있다는 것을 아시나요? 작품의 앞쪽 면에는 요즘 아파트의 컬러 트렌드를 보여주는 파랑, 회색, 갈색 계열 시트지가 붙어 있고, 뒤쪽 면은 건축가 르코르뷔지에가 고안한 20세기 최초의 아파트 ‘유니테 다비타시옹’의 색 구성을 닮았습니다. 르코르뷔지에는 도시 주변부로 밀려난 사람을 위해 아파트를 고안했습니다. 하지만 오늘날 아파트는 그의 의도와는 달리 효율에 따라 수직, 수평의 표준화된 주거공간을 양산하고 있습니다. ");
//        wi.set_work(1,3,"그린 벨트", "김지은","서울 지도에 형형색색 폼폼이가 빽빽하게 붙어 있고 그 주위에는 그린벨트가 조경재료로 꾸며져 있습니다. 폼폼이가 펑! 튀겨나간 듯 서울 주변부로 퍼져 있습니다. 지도 속 알갱이처럼 오늘날 서울에는 절대다수의 인구가 거주하고 있습니다. 작가는 밀집된 색색의 폼폼이들이 폭발해서 그린벨트 바깥으로 터져 나온 듯한 양상을 통해 서울로 과도하게 집중된 사람들의 주거 욕망을 드러냅니다. 그린벨트가 해제되면 수도권의 주거 분포는 어떻게 달라질까요?");
//
//        wi.set_work(2,0,"", "","3관의 시작 지점입니다.");
//        wi.set_work(2,1,"재활용 수거일", "김지은","종이박스로 된 거대한 산이 척박한 화성 풍경 위에 그려져 있습니다. 작가가 사는 아파트 단지는 매주 재활용 수거일마다 쓰레기를 경비의 지도에 따라 일사 정연하게 분리수거한 뒤 쌓아 올려 그림과 같이 정리합니다. 이런 방식은 대단지 아파트에서 막대한 소비로 인해 생기는 쓰레기를 가장 효율적으로 처리하는 방법일 것입니다. 작가는 작품에 실제 상품라벨이 적힌 종이박스를 꼴라주하고 재활용 수거일 풍경을 화성을 배경으로 그림으로써 일상 풍경을 낯설고도 기념비적으로 재현합니다.");
//        wi.set_work(2,2,"옆집", "김지은","작가의 집 베란다 창을 열면 보이는 옆라인 아파트 벽면을 그린 작품입니다. 무난한 색으로 도색된 옆집 벽면은 시간대별 빛의 변화에 따라 달리 보입니다. 작은 사각 모듈로 구성된 옆집 이미지는 아파트 단지를 상공에서 촬영한 구글 이미지를 닮았습니다. 우리 아파트 옆집 벽이지만 어디에서 보아도 만날 수 있을 것 같은, 특유의 구조적이며 질서정연한 모습입니다. 작품은 택지지구 개발 이후에도 여전히 삭막한 현대인의 주거환경을 보여줍니다. ");
//        wi.set_work(2,3,"화성풍경: 모델하우스", "김지은","봉담 택지지구 인근에 있던 모델하우스의 철거 장면을 담은 작품입니다. 아파트 분양을 위해 지어지는 모델하우스는 아직 존재하지 않는 아파트를 실제 공간처럼 완벽하게 재현하여 구매자의 거주 욕망을 가상으로 실현해줍니다. 하지만 그 기능을 다 하고 나면 일시에 철거되어 폐허가 됩니다. 작가는 부동산 광고나 대출 전단지, 인테리어 시트지를 꼴라주하여 모델하우스 철거 풍경을 재현함으로써 아파트라는 공간의 일시성과 가상성을 드러냅니다.");
//    }
//
//
...
//    }
//
//    public String get_wi(int section, int nok){
//        String artist = wi.get_work(section,nok)[1];
//        String work = wi.get_work(section, nok)[0];
//        return artist + "의 " + work;
//    }
//
//    public void speak_wi(int section, int sector){
//        tts.speak(wi.get_work(section, sector)[0] + "은" , TextToSpeech.QUEUE_ADD, null);
//        tts.speak(wi.get_work(section, sector)[2], TextToSpeech.QUEUE_ADD, null);
